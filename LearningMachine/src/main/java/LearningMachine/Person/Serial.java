package LearningMachine.Person;
...
public class Serial
{
    ...
    void connect ( String portName ) throws Exception
    {
        ...
        else
        {
            ...
            if ( commPort instanceof SerialPort )
            {
                ...
                SerialRW rw = new SerialRW(in, out);
                rw.run();
            }
            else
            {
    ...
    public static class FHIRframe
    {
    	 ...
    	 String setUp(String Id, String idenValue, String bodySystem, String bodyCode, String bodyDisplay, String system, String code, String display, String componentCode, String componentDisplay, String unit)
    	 { 
	        ...
	        observer.addIdentifier().setValue(idenValue);
	      	...
	      	concept.addCoding().setSystem(bodySystem).setCode(bodyCode).setDisplay(bodyDisplay);
	      	observer.setBodySite(concept);
	     
	      	
	      	CodingDt coding2 = new CodingDt();
	      	coding2.setSystem(system).setCode(componentCode).setDisplay(componentDisplay);
	      	...
	      	value.setValue(sensorValue).setUnit(unit);
            ...
            try
            {
            	...
            	while (true)
                {
                	if(flag.trim().equals("1"))
                	{
                		...
                		ff.setValue(Integer.parseInt(new String(buffer_int).trim()));
                		sd.wri(ff.setUp("Heart rate by Pulse oximetry", "960924-2895112", "Heart rate measurement site", "8885-6", "Right Index Finger", "Capillary bed", "73795-7", "Heart rate", "73795-7", "Heart rate PulseOX", "{beats}/min"));
                	}
                	else if(flag.trim().equals("2"))
                	{
                		...
                		//ff.setValue(Integer.parseInt(new String(buffer_int).trim()));
                		ff.setValue(Integer.parseInt(new String(buffer_int).trim()));
                		sd.wri(ff.setUp("Oxygen saturation in Arterial blood by Pulse oximetry", "960924-2895112",  "Heart rate measurement site", "8885-6", "Right Index Finger", "BldA", "59408-5", "Oxygen saturation", "59408-5", "SaO2 % BldA PulseOx", "%"));
                	}
